...
def carbonite_received_data_handler(command, value, qualifier):
  ...
  if command == 'ConnectionStatus':
    if   value == 'Connected':
      DebugPrint('devices.py/carbonite_received_data_handler', 'Carbonite has been successfully connected', 'Info')
      ...
      DebugPrint('devices.py/carbonite_received_data_handler', 'Carbonite has been disconnected from the system. Will attempt reconnection..', 'Error')
  ...
  # We use blinking (SetBlinking) to make sure they're noticeable on the screen.
    ...
    DebugPrint('devices.py/carbonite_received_data_handler', 'Unhandled Carbonite Driver Data: [{0}] [{1}] [{2}]'.format(command, value, qualifier), 'Trace')
